...
const ContactInfo: React.FC = () => {
    ...
    return (
        <section id="ContactInfo" className="px-4 py-10 sm:px-10 sm:py-12 lg:px-20 lg:py-20">
            <div className="space-y-10 sm:space-y-12 lg:space-y-16 bg-white px-6 py-8 sm:px-12 sm:py-12 lg:px-16 lg:py-16 rounded-xl">
                {/* Header */}
                <header className="">
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-serif font-medium text-slate-800 mb-3">
                ...
                <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
                    ...
                    <div className="space-y-6 order-2 lg:order-1">
                        <FadeUp className="h-full">
                            <div className="relative shadow-3d-md hover:shadow-3d-lg transition-all duration-300 overflow-hidden rounded-2xl group h-full">
                                <img
                                    src={meetingImage}
                                    alt={lang === 'en' ? 'HLGera Meeting Room' : lang === 'vi' ? 'Phòng họp HLGera' : 'HLGera会议室'}
                                    className="w-full h-60 sm:h-72 lg:h-96 object-cover group-hover:scale-105 transition-transform duration-300"
                                />
                    ...
                    <div className="space-y-6 order-1 lg:order-2">
                        {contactItems.map((item, index) => (
                            <FadeUp key={index}>
                                <div className="bg-white rounded-2xl p-5 sm:p-7 shadow-3d hover:shadow-3d-lg hover:-translate-y-1 transition-all duration-300 border border-slate-100">
                                    <div className="flex items-start gap-3 sm:gap-4">
                                        {/* Icon */}
                                        <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-primary/10 flex items-center justify-center text-xl sm:text-2xl shadow-3d-sm hover:shadow-3d transition-all duration-300">
                                                ...
                                                {item.details.map((detail, detailIndex) => {
                                                    ...
                                                    return (
                                                        <div key={detailIndex} className="flex items-start gap-2">
                                                            <svg
                                                                className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 text-primary mt-0.5"
                                                                fill="none"
                                                                stroke="currentColor"
                                                                viewBox="0 0 24 24"
                                                            >
