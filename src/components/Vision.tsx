...
const Vision = () => {
    const { lang } = useLang()
    return (
        <section id="vision" className="bg-blue-100">
            <div className="">
                <FadeUp className=" border border-slate-200 bg-white from-slate-50 to-blue-50 p-8 sm:p-12">
                    <h2 className="text-3xl sm:text-4xl font-bold text-orange-500 tracking-tight font-serif">{lang === 'en' ? 'Vision' : lang === 'vi' ? 'Tầm Nhìn' : '愿景'}</h2>
                    <div className="mt-6 text-center">
                        <p className="mx-auto max-w-5xl text-xl sm:text-2xl leading-relaxed font-serif text-blue-700">
                    ...
                    <div className="mt-10 grid gap-8 lg:grid-cols-2">
                        <div className="shadow-3d shadow-3d-hover rounded-[32px] border-2 border-primary/20 bg-white backdrop-blur-sm p-6 sm:p-8 transition-shadow">
                            <h3 className="text-2xl font-serif font-semibold text-orange-500">{lang === 'en' ? 'Strategic Directions' : lang === 'vi' ? 'Định Hướng Chiến Lược' : '战略方向'}</h3>
                            <ul className="mt-6 space-y-3 text-base text-blue-500">
