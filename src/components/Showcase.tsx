...
interface ProjectCardProps {
    p: { id: ProjectId, title: string, description: string, image: string };
    reverse?: boolean;
}

const translateProject = (id: ProjectId, defaultTitle: string, defaultDescription: string, lang: 'vi' | 'en' | 'zh') => {
    if (lang === 'vi') return { title: defaultTitle, description: defaultDescription }
    if (lang === 'en') {
        const map: Record<ProjectId, { t: string; d: string }> = {
            'coursePlatform': {
        ...
        if (map[id]) return { title: map[id].t, description: map[id].d }
    }
    if (lang === 'zh') {
        const map: Record<ProjectId, { t: string; d: string }> = {
            'coursePlatform': {
...
const getProjectBullets = (id: ProjectId, lang: 'vi' | 'en' | 'zh') => {
    // Build bullet points per project and language
    const vi: Record<ProjectId, string[]> = {
        'coursePlatform': [
    ...
    const en: Record<ProjectId, string[]> = {
        'coursePlatform': [
    ...
    const zh: Record<ProjectId, string[]> = {
        'coursePlatform': [
            '连接专家/创作者与联盟推广者',
            '支持学员高效在线学习',
            '集成支付：VNPay、MoMo'
    ...
    if (lang === 'vi' && vi[id]) return vi[id]
    if (lang === 'en' && en[id]) return en[id]
    if (lang === 'zh' && zh[id]) return zh[id]
    // Fallback to generic
    return lang === 'en'
...
const getProjectPills = (id: ProjectId, lang: 'vi' | 'en' | 'zh') => {
    const vi: Record<ProjectId, string[]> = {
        'coursePlatform': ['Affiliate', 'Laravel/Vue', 'Thanh toán'],
        'rideHailing': ['iOS/Android', 'Điều phối thông minh', '< 2s'],
        'elearningPlatform': ['Quản lý khóa học', 'Tương tác', 'Đánh giá'],
        'enterpriseAiChatbot': ['Hỗ trợ 24/7', 'RASA NLP', 'Tích hợp đa kênh'],
        'smartMedicalRecords': ['OCR ~95%', '15 BV + 50+ PK', 'TensorFlow/OpenCV'],
        'binanceBot': ['AI Trading', '10K+ Users', 'Node.js/React Native'],
    }
    const en: Record<ProjectId, string[]> = {
        'coursePlatform': ['Affiliate', 'Laravel/Vue', 'Payments'],
        'rideHailing': ['iOS/Android', 'Smart dispatch', '< 2s'],
        'elearningPlatform': ['Course mgmt', 'Interaction', 'Assessment'],
        'enterpriseAiChatbot': ['24/7 support', 'RASA NLP', 'Omnichannel'],
        'smartMedicalRecords': ['OCR ~95%', '15 hospitals+50 clinics', 'TensorFlow/OpenCV'],
        'binanceBot': ['AI Trading', '10K+ Users', 'Node.js/React Native'],
    }
    const zh: Record<ProjectId, string[]> = {
        'coursePlatform': ['联盟推广', 'Laravel/Vue', '支付'],
        'rideHailing': ['iOS/Android', '智能调度', '< 2 秒'],
        'elearningPlatform': ['课程管理', '互动', '评测'],
        'enterpriseAiChatbot': ['7×24', 'RASA NLP', '全渠道'],
        'smartMedicalRecords': ['OCR ~95%', '15 医院+50 诊所', 'TensorFlow/OpenCV'],
        'binanceBot': ['AI 交易', '10K+ 用户', 'Node.js/React Native'],
    }
    if (lang === 'vi' && vi[id]) return vi[id]
    if (lang === 'en' && en[id]) return en[id]
    if (lang === 'zh' && zh[id]) return zh[id]
...
const ProjectCard: React.FC<ProjectCardProps> = ({ p, reverse }) => {
    const { lang } = useLang()
    const tp = translateProject(p.id, p.title, p.description, lang)
    const pills = getProjectPills(p.id, lang)
    const bullets = getProjectBullets(p.id, lang)
    return (
            ...
            <div className={`grid md:grid-cols-2 gap-6 items-center p-6`}>
                ...
                <div className={`flex flex-col gap-3 sm:gap-4 ${reverse ? 'md:order-1' : 'md:order-2'}`}>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                        {pills.map((x, idx) => (
                            <span key={idx} className="px-3 py-1  text-xs sm:text-sm bg-primary/10 text-primary ring-1 ring-primary/20">{x}</span>
                        ))}
                    </div>
                </div>
            </div>
        ...
        <section id="showcase" className=" w-[80%] bg-white py-7"   >
            <div className="flex flex-col gap-6 sm:gap-8 w-full ">
                ...
                <FadeUp className="flex flex-col gap-10 relative w-full max-w-7xl mx-auto px-2 pb-4 sm:pb-6 lg:pb-8">
                    <>
                        {projects.map((p, i) => (
                            <ProjectCard key={p.id} p={p} reverse={i % 2 === 1} />
                        ))}
                    </>
                </FadeUp>
            </div>
